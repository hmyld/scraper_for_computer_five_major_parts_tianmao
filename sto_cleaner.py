...
INPUT_FILE = "data/disk_result.json"
CLEANED_FILE = "data/disk_result_cleaned.json"
SUMMARY_FILE = "data/disk_price_summary.json"

MAX_PRICE = 50000  # 企业级硬盘/大容量SSD可能很贵
MIN_SAMPLE = 3
...
def main():
    ...
    # 按 storage_key + brand 分组
    groups = defaultdict(list)
    storage_all_prices = defaultdict(list)
    for d in data:
        try:
            price = float(d.get("price", 0))
            if price > 0:
                key = (d["storage_key"], d.get("brand", "未知"))
                groups[key].append(d)
                storage_all_prices[d["storage_key"]].append(price)
        except:
            continue
    ...
    print(f"\n{'规格':<30} {'品牌':<8} {'原始':>4} {'删除':>4} {'保留':>4} {'σ':>4} {'最低':>7} {'中位':>7} {'均值':>7} {'最高':>7} {'推荐价':>8}")
    print("-" * 110)

    for (storage_key, brand), items in sorted(groups.items()):
        ...
        total = len(storage_all_prices[storage_key])
        ...
        summary[storage_key][brand] = {
            "storage_total_sample": total,
        ...
        print(f"{storage_key:<30} {brand:<8} {len(items):>4} {removed_count:>4} {len(kept_items):>4} "
              f"{best_sigma:>4.1f} "
              f"{min_p:>7.0f} "
              f"{median:>7.0f} "
              f"{mean:>7.0f} "
    ...
    # 规格整体汇总
    print(f"\n{'='*80}")
    print(f"📊 存储规格整体价格（不区分品牌）")
    print(f"{'='*80}")
    print(f"{'规格':<30} {'样本':>4} {'最低':>7} {'25%':>7} {'中位':>7} {'均值':>7} {'75%':>7} {'最高':>7} {'推荐':>7}")
    print("-" * 95)

    storage_summary = {}
    for storage_key in sorted(storage_all_prices.keys()):
        all_prices = storage_all_prices[storage_key]
        ...
        print(f"{storage_key:<30} {total:>4} "
    ...
    final_output = {}
    for storage_key in sorted(storage_summary.keys()):
        final_output[storage_key] = {
            "storage_summary": storage_summary[storage_key],
            "brands": summary[storage_key]
        }
    ...
    low_sample_count = sum(1 for v in storage_all_prices.values() if len(v) < MIN_SAMPLE)
    ...
    print(f"   覆盖规格: {len(storage_summary)} 种")
