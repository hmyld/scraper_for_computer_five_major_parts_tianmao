...
INPUT_FILE = "data/gpu_result.json"
...
def main():
    ...
    # 同时按芯片汇总所有价格
    chip_all_prices = defaultdict(list)
    for d in data:
        try:
            price = float(d.get("price", 0))
            if price > 0:
                key = (d["chip_model"], d.get("brand", "未知"))
                groups[key].append(d)
                chip_all_prices[d["chip_model"]].append(price)
        except:
            continue
    ...
    print(f"\n{'芯片型号':<18} {'品牌':<8} {'原始':>4} {'删除':>4} {'保留':>4} {'σ':>4} {'最低':>7} {'中位':>7} {'均值':>7} {'最高':>7} {'推荐价':>10}")
    print("-" * 105)

    low_sample_chips = 0

    for (chip_model, brand), items in sorted(groups.items()):
        ...
        total = len(chip_all_prices[chip_model])
        ...
        if brand_stats:
            ...
            if is_low_sample:
                ...
                recommend_str = f"{recommend:>7.0f}"
        else:
            median = mean = min_p = max_p = 0
            recommend = None
            recommend_str = "⚠️样本不足"

        summary[chip_model][brand] = {
            "chip_total_sample": total,
        ...
        print(f"{chip_model:<18} {brand:<8} {len(items):>4} {removed_count:>4} {len(kept_items):>4} "
              f"{best_sigma:>4.1f} "
              f"{min_p:>7.0f} "
              f"{median:>7.0f} "
              f"{mean:>7.0f} "
    ...
    # ========== 新增：芯片整体价格汇总 ==========
    print(f"\n{'='*60}")
    print(f"📊 芯片整体价格汇总（不区分品牌）")
    print(f"{'='*60}")
    print(f"{'芯片型号':<18} {'样本':>4} {'最低':>7} {'25%':>7} {'中位':>7} {'均值':>7} {'75%':>7} {'最高':>7} {'推荐价':>7}")
    print("-" * 80)

    chip_summary = {}
    for chip_model in sorted(chip_all_prices.keys()):
        all_prices = chip_all_prices[chip_model]
    ...
    # 最终输出结构：芯片汇总 + 品牌明细
    final_output = {}
    for chip_model in sorted(chip_summary.keys()):
        final_output[chip_model] = {
            "chip_summary": chip_summary[chip_model],
            "brands": summary[chip_model]
        }
    ...
    low_sample_chips = sum(1 for v in chip_all_prices.values() if len(v) < MIN_SAMPLE)
    total_removed = len(data) - len(cleaned_data) + removed_high
    ...
    print(f"   覆盖芯片: {len(chip_summary)} 款")
    print(f"   ⚠️ 样本不足芯片: {low_sample_chips} 款（总样本<{MIN_SAMPLE}条）")
